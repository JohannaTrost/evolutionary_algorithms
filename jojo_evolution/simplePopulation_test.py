...
def crossing(parents, pop_size):
    ...
    assert(len(next_generation) == len(all_parents))
...
for i in range(num_generations):
    ...
    assert(len(curr_population) == len(distances))
    ...
    if (i < num_generations - 1):
        ...
        curr_population, parent_ids = crossing(parents_selected, pop_size)
        all_parent_ids.append(parent_ids)
        disable_collision(curr_population)

        generations.append(curr_population)

assert(num_generations == len(all_distances))
assert(num_generations == len(all_parent_ids))
assert(num_generations == len(generations))
